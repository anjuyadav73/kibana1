...
describe("#setSearchTerm", () => {
    function subject(term, { autocompleteResults, autocompleteSuggestions, refresh, initialState = {}, shouldClearFilters } = {}) {
    ...
    (0, sharedTests_1.itResetsCurrent)(() => subject("test", { initialState: { current: 2 } }).state);
    (0, sharedTests_1.itResetsFilters)(() => subject("test", {
        initialState: {
            filters: [
                {
                    field: "filter1",
    ...
    it("Does not update other Search Parameter values", () => {
        ...
        const { resultsPerPage, sortField, sortDirection, sortList } = subject("test", {
            initialState
        }).state;
        expect({ resultsPerPage, sortField, sortDirection, sortList }).toEqual(initialState);
    });
    (0, sharedTests_1.itFetchesResults)(() => subject("term").state);
    ...
    it("Does not update URL state when 'refresh' is set to false", () => {
        ...
        expect(MockedURLManager.mock.instances[0].pushStateToURL.mock
    ...
    describe("when autocompleteResults is true", () => {
        it("updates autocompletedResults", () => {
            expect(subject("term", { autocompleteResults: true, refresh: false }).state
                .autocompletedResults).toEqual([{}, {}]);
        });
        it("Will not debounce requests if there is no debounce specified", () => {
            ...
            expect((0, helpers_1.getAutocompleteCalls)(mockApiConnector)).toHaveLength(3);
        });
        it("Will debounce requests", () => {
            ...
            jest.runAllTimers();
            expect((0, helpers_1.getAutocompleteCalls)(mockApiConnector)).toHaveLength(1);
        });
        describe("and autocompleteMinimumCharacters is set and less than requirement", () => {
            it("will not trigger", () => {
                ...
                expect((0, helpers_1.getAutocompleteCalls)(mockApiConnector)).toHaveLength(0);
            });
        });
    ...
    describe("when autocompleteSuggestions is true", () => {
        it("updates autocompletedSuggestions", () => {
            expect(subject("term", { autocompleteSuggestions: true, refresh: false }).state
                .autocompletedSuggestions).toEqual({
                documents: [
                    { suggestion: "carlsbad" },
    ...
    describe("when autocompleteResults is false", () => {
        it("doesn't update autocompletedResults", () => {
            expect(subject("term", { autocompleteResults: false }).state
                .autocompletedResults).toEqual([]);
        });
    });
    describe("when autocompleteSuggestions is false", () => {
        it("doesn't update autocompletedSuggestions", () => {
            expect(subject("term", { autocompleteSuggestions: false }).state
                .autocompletedSuggestions).toEqual({});
        });
    });
});
