...
function multilayerAxisEntry(xDomain, extendByOneBin, range, timeAxisLayerCount, scale, getMeasuredTicks) {
    ...
    var binStartsFrom = domainFromS - binWidthS;
    var binStartsTo = domainToS + binWidthS;
    return layers.reduce(function (combinedEntry, l, detailedLayerIndex) {
        if (l.labeled)
            layerIndex++;
        if (layerIndex >= timeAxisLayerCount)
            return combinedEntry;
        var timeTicks = __spreadArray([], __read(l.binStarts(binStartsFrom, binStartsTo)), false).filter(function (b) { return b.nextTimePointSec > domainFromS && b.timePointSec <= domainToS; })
            .map(function (b) { return 1000 * b.timePointSec; });
        var entry = fillLayerTimeslip(layerIndex, detailedLayerIndex, timeTicks, !l.labeled ? function () { return ''; } : layerIndex === timeAxisLayerCount - 1 ? l.detailedLabelFormat : l.minorTickLabelFormat, (0, exports.notTooDense)(domainFromS, domainToS, binWidth, Math.abs(range[1] - range[0]), MAX_TIME_GRID_COUNT)(l)).entry;
        var minLabelGap = 4;
