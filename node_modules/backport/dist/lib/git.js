...
async function getGitProjectRootPath(dir) {
    try {
        ...
        return path_1.default.normalize(stdout.trim());
    }
    catch (e) {
...
async function getIsCommitInBranch(options, commitSha) {
    try {
        ...
        logger_1.logger.warn('getIsCommitInBranch threw', e);
        return false;
    }
}
...
async function getIsMergeCommit(options, sha) {
    ...
    try {
        ...
        logger_1.logger.info(`Could not determine if ${shortSha} is a merge commit. Will assume it is not`, e);
        return false;
    }
}
