...
describe('entrypoint.module', () => {
    describe('backportRun', () => {
        describe('when running into merge conflict', () => {
            let response;
            beforeAll(async () => {
                response = (await (0, entrypoint_api_1.backportRun)({
                    options: {
            ...
            it('should have correct error code', () => {
                //@ts-expect-error
                expect(response.results[0].error.errorContext.code).toBe('merge-conflict-exception');
                //@ts-expect-error
                expect(response.results[0].error.message).toBe('Commit could not be cherrypicked due to conflicts in: la-liga.md');
            });
            it('contains a list of conflicting files', () => {
                //@ts-expect-error
                expect(response.results[0].error.errorContext.conflictingFiles).toEqual(['la-liga.md']);
            });
        });
        ...
        describe('when missing branches to backport to', () => {
            ...
            it('should correct error code', async () => {
                ...
                expect(response.error.message).toBe('There are no branches to backport to. Aborting.');
            });
        });
    ...
    describe('getCommits', () => {
        it('pullNumber', async () => {
            const commits = await (0, entrypoint_api_1.getCommits)({
            ...
            const expectedCommits = [
                {
                    ...
                    sourceCommit: {
                        ...
                        message: '[APM] Fix incorrect table column header (95th instead of avg) (#88188)',
                    ...
                    targetPullRequestStates: [
                        {
                            ...
                            labelRegex: '^v(\\d+).(\\d+).\\d+$',
                            mergeCommit: {
                                message: '[APM] Fix incorrect table column header (95th instead of avg) (#88188) (#88289)',
                        ...
                        },
                        {
                            branch: '7.x',
                            mergeCommit: {
                                message: '[7.x] [APM] Fix incorrect table column header (95th instead of avg) (#88188) (#88288)\n\nCo-authored-by: Kibana Machine <42973632+kibanamachine@users.noreply.github.com>',
        ...
        it('prFilter', async () => {
            ...
            const commitMessage = commits.map((commit) => {
                return {
                    ...commit.sourceCommit,
                    message: (0, commitFormatters_1.getFirstLine)(commit.sourceCommit.message),
      ...
      `);
        ...
        it('throws when missing a filter', async () => {
            await expect(() => (0, entrypoint_api_1.getCommits)({
                accessToken: accessToken,
                repoName: 'kibana',
                repoOwner: 'elastic',
                maxNumber: 3,
            })).rejects.toThrow('Must supply one of: `pullNumber`, `sha`, `prFilter` or `author`');
        });
    });
});
//# sourceMappingURL=entrypoint.module.private.test.js.map