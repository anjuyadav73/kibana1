...
const packageVersion = __importStar(require("../../../utils/packageVersion"));
...
describe('entrypoint cli', () => {
    it('--version', async () => {
        const { output } = await (0, runBackportViaCli_1.runBackportViaCli)([`--version`], {
            showOra: true,
        });
        expect(output).toEqual(process.env.npm_package_version);
    });
    it('-v', async () => {
        const { output } = await (0, runBackportViaCli_1.runBackportViaCli)([`-v`], {
            showOra: true,
        });
        expect(output).toEqual(process.env.npm_package_version);
    });
    it('PACKAGE_VERSION should match', async () => {
        // @ts-expect-error
        expect(packageVersion.UNMOCKED_PACKAGE_VERSION).toBe(process.env.npm_package_version);
    });
    it('--help', async () => {
        const { output } = await (0, runBackportViaCli_1.runBackportViaCli)([`--help`]);
    ...
    it('lists commits based on .git/config when `repoOwner`/`repoName` is missing', async () => {
        ...
        await (0, child_process_promisified_1.exec)(`git remote add origin git@github.com:backport-org/backport-e2e.git`, { cwd: sandboxPath });
        const { output } = await (0, runBackportViaCli_1.runBackportViaCli)([`--accessToken=${accessToken}`], {
            cwd: sandboxPath,
...
//# sourceMappingURL=entrypoint.cli.private.test.js.map