...
const cli = yargs_1.default
    ...
    .usage('Usage: apidoc-markdown -p <path> -o <output_file> [-t <template_name>] [--multi] [--createPath] [--prepend <file_path>]')
    .example('apidoc-markdown -p doc/ -o doc.md', 'Generate from `doc/` apiDoc output to `./doc.md`')
    .example('apidoc-markdown -p doc/ -o doc.md -t bitbucket', 'Generate from `doc/` apiDoc output to `./doc.md` using the bitbucket template')
    .example('apidoc-markdown -p doc/ -o doc.md -t ./mytemplate.md', 'Generate from `doc/` apiDoc output to `./doc.md` using a provided template file')
    ...
    .option('template', {
    alias: 't',
    describe: 'Name of the template to be used (`default`, `bitbucket`) or path to an EJS template file. If not specified, the default template is used',
    default: 'default',
    type: 'string'
})
    .option('header', {
